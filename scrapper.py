...
from save_functions import save_url, save_audio, save_image,  save_text_to_file, save_document, save_text_to_file  #scrape_website,
...
def create_new():
    ...
    return(jsonify({'status': 'success', 'message': 'Data saved successfully', 'client_id':client_id}))
...
def upload_urls():
    ...
    return jsonify({'status': 'success', 'message': 'URL uploaded successfully', 'client_id': client_id, 'scraped_data': return_text_content,  'milvus_data': return_df})
...
def process_urls(client_id, urls, max_depth):
    with app.app_context():  
        ...
        for url in urls:
            ...
            scrapped_text1, link_status, json_data = scrape_website(url, max_depth, client_id)
            scrapped_text = " ".join(scrapped_text1)
        ...
        return jsonify({'status': 'success', 'message': 'URLs uploaded and processed successfully', 'results': results})
...
def upload_url():
    ...
    urls = data.get('urls', [])
    max_depth = data.get('max_depth')
    frequency = data.get('frequency', 'minute')  # Default here as minute

    if frequency == 'minute':
        scheduler.add_job(process_urls, 'interval', minutes=1, args=[client_id, urls, max_depth])
    elif frequency == 'hour':
        scheduler.add_job(process_urls, 'interval', hours=1, args=[client_id, urls, max_depth])
    elif frequency == 'second':
        scheduler.add_job(process_urls, 'interval', seconds=5, args=[client_id, urls, max_depth])
    elif frequency == 'day':
        scheduler.add_job(process_urls, 'interval', days=1, args=[client_id, urls, max_depth])
    else:
        return jsonify({'status': 'error', 'message': 'Invalid frequency'})

    return jsonify({'status': 'success', 'message': 'URLs scheduled for processing'})
...
def ask():
    ...
    requestData = request.json
    # chatHistory = requestData.get('chatHistory')
    # client_cred = requestData.get("client_cred")
    # vectordb_cred = requestData.get("vectordb_cred")
    # llm_cred = requestData.get("llm_cred")
    # client_id = client_cred["client_name"]+"_"+client_cred["project_name"]
