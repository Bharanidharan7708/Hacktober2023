...
def scrape_website_file_1(url, max_depth, client_name):
    # print("Scraping started for client_id:", client_name)
    # print("Scraping URL:", url)
    file_text = scrape_website_files(url, max_depth, client_name)
 
    last_run_file = get_latest_json_file("extracted_file_data_json/")
    save_data_file(file_text)
    time.sleep(2)  # Wait for 2 seconds to ensure the file is saved
    current_run_file = get_latest_json_file("extracted_file_data_json/")
...
if __name__ == "__main__":
    ...
    start_url="https://research.ibm.com/blog/retrieval-augmented-generation-RAG"
    depth = 2
    client_name = "client_01"
 
    scrape_website(start_url, depth, client_name)
    scrape_website_file_1(start_url, depth, client_name)
